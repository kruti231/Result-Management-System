...
class studentClass:
    def __init__(self,root):
        ...
        self.txt_gender=ttk.Combobox(self.root,textvariable=self.var_gender,values=("Select option","Male","Female","Other"),font=("goudy old style",15,"bold"),state='readonly',justify=CENTER)
        ...
        btn_search=Button(self.root,text="Search",font=("goudy old style",15,"bold"),bg="#03a9f4",fg="white",cursor="hand2",command=self.search).place(x=1060,y=60,width=120,height=28)
        ...
        self.CourseTable=ttk.Treeview(self.C_Frame,columns=("roll","name","email","gender","dob","contact","admission","course","city","state","pin","address"),xscrollcommand=Scrollx.set,yscrollcommand=Scrolly.set)
        ...
        try:
            if self.var_roll.get()=="":
                ...
                if row!=None:
                    ...
                    cur.execute("insert into student (roll,name,email,gender,dob,contact,admission,course,city,state,pin,address) values(?,?,?,?,?,?,?,?,?,?,?,?)",(
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
        ...
        try:
            if self.var_roll.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("update student set name=?,email=?,gender=?,dob=?,contact=?,admission=?,course=?,city=?,state=?,pin=?,address=?,roll=?",(
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
                        self.var_dob.get(),
